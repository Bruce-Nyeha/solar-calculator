...
export default class ExternalServices {
  ...
  async getPeakSunHours(cityKey) {
    ...
    try {
      ...
      const totalKwh = radiationArray.reduce(
        (sum, value) => sum + value * 0.277778,
        0,
      );
  ...
  async getLiveExchangeRate() {
    ...
    if (!apiKey) {
      console.warn(
        "API Key configuration error inside .env profile layout mapping.",
      );
      return 15.25;
    }
    ...
    try {
      const response = await fetch(url);
      if (!response.ok)
        throw new Error("Secure ExchangeRate-API connection failure.");
      const data = await response.json();

      return Number(data.conversion_rates.GHS.toFixed(2));
    } catch (error) {
      console.warn(
        "Currency fallback activated due to secure verification blocks:",
        error,
      );
      return 15.25;
    }
